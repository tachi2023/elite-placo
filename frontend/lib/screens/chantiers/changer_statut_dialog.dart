...
class ChangerStatutDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final transitions =
        StatutChantier.transitionsValides[chantier.statut] ?? [];

    return AlertDialog(
      backgroundColor: AppTheme.anthraciteClair,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(16),
        side: BorderSide(color: AppTheme.or.withValues(alpha: 0.3)),
      ),
      ...
      content: transitions.isEmpty
          ? const Text('Aucune transition disponible depuis ce statut.',
              style: TextStyle(color: Colors.white))
          : Column(
              mainAxisSize: MainAxisSize.min,
              children: transitions
                  .map((statut) => Container(
                        ...
                        child: ListTile(
                          ...
                          trailing: const Icon(Icons.arrow_forward_ios,
                              size: 16, color: AppTheme.or),
                          onTap: () async {
                            final provider = context.read<ChantierProvider>();
                            final ok = await provider.changerStatut(
                                chantier.id!, statut);
                            ...
                            if (!ok) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(provider.derniereErreur ??
                                      'Erreur inconnue.'),
